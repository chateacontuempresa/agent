...
import { DocumentSyncManager } from './documentSync.js';

const { Client: PgClient } = pg;

const VERSION = '1.0.0';
/** Alineado con LARGE_READ_CAP en el SaaS (lecturas masivas / CSV). */
const AGENT_READ_CAP = 100000;
...
const {
  ...
  db_user,
  db_pass = '',
  document_sync_poll_minutes = 5,
} = config;

const documentSync = new DocumentSyncManager((level, message) => {
  const tag = level === 'error' ? '[ERROR]' : level === 'warn' ? '[WARN]' : '[Sync]';
  ...
  function connect() {
    ...
    ws.on('message', async (raw) => {
      ...
      if (msg.type === 'agent.schema.sync.ok') {
        ...
        documentSync.startMappingsPoll(Math.max(1, Number(document_sync_poll_minutes) || 5) * 60 * 1000);
        return;
      }

      if (msg.type === 'agent.sync.mappings' || msg.type === 'agent.sync.mappings.update') {
        documentSync.applyMappings(msg.mappings || []);
        return;
      }
      ...
      if (msg.type === 'agent.query.execute') {
        ...
        if (!/\bLIMIT\b/i.test(sql) && !/\bCOUNT\s*\(/i.test(sql)) {
          finalSql =
            db_type === 'mssql'
              ? sql.replace(/^SELECT\b/i, `SELECT TOP ${AGENT_READ_CAP}`)
              : `${sql} LIMIT ${AGENT_READ_CAP}`;
        }
    ...
    ws.on('close', (code, reason) => {
      documentSync.clearMappingsPoll();
      void documentSync.stopAll();
      documentSync.setSocket(null);
