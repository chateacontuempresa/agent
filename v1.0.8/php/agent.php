...
/** Debe alinearse con LARGE_READ_CAP en el SaaS (lecturas masivas / CSV). */
define('CCE_AGENT_READ_CAP', 100000);
...
require_once __DIR__ . '/document_sync.php';
...
function runAgentCycle(bool $cli = false): void {
    // ─── BUG FIX #1: Prevenir que PHP mate el script por max_execution_time ──
    // Hosting compartido suele tener max_execution_time = 30s.
    // El ciclo dura ~56s → sin esto PHP mataría el proceso a los 30s cada vez.
    @set_time_limit(0);
    @ini_set('max_execution_time', '0');
    // Si el cron corre vía URL HTTP y el caller cierra la conexión,
    // ignore_user_abort(true) permite que el script siga corriendo de todas formas.
    ...
    // ─── BUG FIX #2: Auth con reintentos ─────────────────────────────────────
    // Antes: si auth fallaba por un hipo de red → ciclo entero abortaba.
    // Ahora: hasta 3 intentos con 2s de pausa entre ellos.
    ...
    for ($authAttempt = 1; $authAttempt <= 3; $authAttempt++) {
        $auth = gatewayRequest('POST', '/api/http-agent/auth', $tempCfg, $authPayload, 20);
        if (!empty($auth['ok'])) break;
        $httpCode = (int)($auth['_http_code'] ?? 0);
        // 401 = token inválido → no tiene sentido reintentar
        if ($httpCode === 401) break;
        if ($authAttempt < 3) {
            if ($cli) echo "[WARN] Auth intento {$authAttempt} falló (HTTP {$httpCode}), reintentando en 2s...\n";
            sleep(2);
        }
    }
    ...
    if (empty($auth['ok'])) {
        ...
        $httpCode = (int)($auth['_http_code'] ?? 0);
        // Si falló por error de red (no 401), limpiar gateway_url para re-bootstrap en la próxima ejecución
    ...
    try {
        ...
        // No abortamos: podemos seguir escuchando consultas aunque la BD falle al arrancar
    }

    // Sincronización de documentos locales → nube (HTTP, sin WebSocket)
    try {
        cce_document_sync_run($cfg, $tempCfg, $cli);
    } catch (\Throwable $e) {
        addLog('error', 'Document sync: ' . $e->getMessage());
        if ($cli) echo '[WARN] Document sync: ' . $e->getMessage() . "\n";
    }

    // ─── BUG FIX #3: Crear conexión PDO una sola vez fuera del loop ──────────
    // Antes: nueva conexión por cada query → ineficiente y puede agotar el pool.
    // Ahora: una conexión reutilizada, con reconexión automática si se pierde.
    $pdo = null;
    try {
        $pdo = getDbConnection($cfg);
    } catch (\Exception $e) {
        addLog('warn', 'BD no disponible al inicio del ciclo: ' . $e->getMessage());
        if ($cli) echo "[WARN] BD no disponible al inicio: " . $e->getMessage() . "\n";
        // Continuamos: si llega una query, intentaremos reconectar en ese momento
    }

    // Multi-query loop
    $cycleStart      = microtime(true);
    $maxCycleSeconds = 56; // Aumentado de 52 → 56 para cubrir mejor el intervalo del cron
    $queriesHandled  = 0;

    // ─── BUG FIX #4: Heartbeat periódico ─────────────────────────────────────
    // El gateway solo actualiza lastSeenAt en auth y heartbeat (NO en poll/result).
    // Con ventana de 90s y cron cada 60s, un delay de 31s en el cron → agente OFFLINE.
    // Solución: llamar /heartbeat cada ~40s durante el ciclo para mantener lastSeenAt fresco.
    $lastHeartbeatAt = time();
    $heartbeatEvery  = 40; // segundos
    ...
    while (true) {
        ...
        if ((time() - $lastHeartbeatAt) >= $heartbeatEvery) {
            $hbRes = gatewayRequest('POST', '/api/http-agent/heartbeat', $tempCfg, [
                'platform'   => 'php-hosting',
                'appVersion' => CCE_VERSION,
            ], 10);
            if (!empty($hbRes['ok'])) {
                if ($cli) echo "[INFO] Heartbeat OK — " . ($hbRes['at'] ?? '') . "\n";
            } else {
                if ($cli) echo "[WARN] Heartbeat falló: " . ($hbRes['error'] ?? '?') . "\n";
            }
            $lastHeartbeatAt = time();
        }

        // Recalcular remaining después del heartbeat
        $elapsed   = microtime(true) - $cycleStart;
        $remaining = $maxCycleSeconds - $elapsed;
        if ($remaining < 3) break;
        ...
        $pollTimeout = (int) ($pollMs / 1000) + 12; // +12s buffer (antes era +5s — muy justo)

        if ($cli) echo "[INFO] Poll — {$pollMs}ms (quedan " . round($remaining) . "s)...\n";

        $pollRes = gatewayRequest('GET', "/api/http-agent/poll?timeout={$pollMs}", $tempCfg, [], $pollTimeout);

        // ─── BUG FIX #5: Manejo de errores de red en el poll ─────────────────
        // Antes: error de curl → código intentaba acceder a $pollRes['query'] de todos modos
        // y abortaba silenciosamente sin log útil.
        // Ahora: detectamos error de red, logueamos y continuamos el ciclo.
        if (!empty($pollRes['error']) && ($pollRes['_http_code'] ?? 0) === 0) {
            addLog('warn', 'Poll - error de red: ' . $pollRes['error']);
            if ($cli) echo "[WARN] Poll error de red: {$pollRes['error']} — continuando ciclo...\n";
            sleep(2); // Pequeña pausa antes de reintentar
            continue;
        }

        // 401 durante el poll = sesión expirada → re-auth en próxima ejecución
        if (($pollRes['_http_code'] ?? 0) === 401) {
            addLog('warn', 'Poll recibió 401 — sesión expirada, re-auth en próxima ejecución.');
            if ($cli) echo "[WARN] Poll 401 — sesión expirada.\n";
            break;
        }
        ...
        try {
            // ── Reconectar PDO si la conexión fue cerrada por el servidor ────
            if ($pdo === null) {
                $pdo = getDbConnection($cfg);
                if ($cli) echo "[INFO] Reconectando a la BD...\n";
            }
            $sqlToRun = $sql;
            if (!preg_match('/\bLIMIT\b/i', $sqlToRun) && !preg_match('/\bCOUNT\s*\(/i', $sqlToRun)) {
                $sqlToRun .= ' LIMIT ' . (int) CCE_AGENT_READ_CAP;
            }
            ...
            $errMsg = $e->getMessage();
            // Si la conexión se cayó, limpiar $pdo para reconectar en la próxima query
            $connLost = stripos($errMsg, 'server has gone away') !== false
                     || stripos($errMsg, 'Lost connection') !== false
                     || stripos($errMsg, 'SQLSTATE[HY000]') !== false
                     || stripos($errMsg, 'no connection') !== false;
            if ($connLost) {
                $pdo = null;
                addLog('warn', 'Conexión BD perdida, se reconectará en la próxima consulta.');
                if ($cli) echo "[WARN] Conexión BD perdida — se reconectará.\n";
            }
            gatewayRequest('POST', '/api/http-agent/result', $tempCfg, [
                'queryId' => $queryId,
                'success' => false,
...
if ($action === 'webhook' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!preg_match('/\bLIMIT\b/i', $sql) && !preg_match('/\bCOUNT\s*\(/i', $sql)) {
            $sql .= ' LIMIT ' . (int) CCE_AGENT_READ_CAP;
        }
